...
-- question 3.2
-- im not sure if it wants the chefs that participated with the given cuisine that year
-- or if it wants any chef that has this cuisine and participated that year
-- so this is yours

-- Asuumption is that we want every chef from a certain cuisine that was assigned
-- a recipe from that cuisine in every season, also maybe remove ids?
...
-- unoptimised version with force index
-- This essentially forces the database to search with unrelated indexes, blocking it from using refrences and forcing checking significantly more rows
EXPLAIN
WITH label_pairs AS (
    SELECT 
        r1.REC_ID,
...
WITH PlayerClasses AS (
    SELECT
        e.EP_ID,
        COUNT(DISTINCT c.Class) AS PlayerClassCount
    FROM
        episodes e
    LEFT JOIN
...
JudgeClasses AS (
    SELECT
        e.EP_ID,
        COUNT(DISTINCT c.Class) AS JudgeClassCount
    FROM
        episodes e
    LEFT JOIN
