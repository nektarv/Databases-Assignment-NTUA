...
DROP DATABASE IF EXISTS cooking_contestdb;

CREATE DATABASE cooking_contestdb;
...
CREATE TABLE `chefs` (
  ...
  CONSTRAINT `chefs_chk_2` CHECK ((`Experience` < `Age`-17)),
...
CREATE TABLE `plays` (
  ...
  CONSTRAINT `playerscore_1` CHECK ((`score1` < 6)),
  CONSTRAINT `playerscore_2` CHECK ((`score2` < 6)),
  CONSTRAINT `playerscore_3` CHECK ((`score3` < 6)),
...
DECLARE username VARCHAR(255);
	...
	SET @sql_query = CONCAT('SET DEFAULT ROLE `Chef` TO  \'',username,'\'@\'localhost\';');
